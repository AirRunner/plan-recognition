...
for map in ("Enigma", "WinterConquest", "BigGameHunters", "Caldera"):
    # Load map
    world = World(map)
    ...
    for i in range(len(world.observations)):
        ...
        correct = argmax(probs) == argmax(world.labels[i])
...
\n")
        ...
        canvas.create_rectangle(590, 10, 630, 50, fill='#424242', width=0)
        canvas.create_text(
            (608, 30),
            text='✓' if correct else '✗',
            fill="#00FF00" if correct else "#FF0000",
            font=custom_font
        )
